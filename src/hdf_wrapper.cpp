                    ...
                    hid_t dtype_file)
{
  ...
  if (!(NULL == buf || 0 == dims[0]))
  {
    ...
    if (status < 0)
    {
      ...
      throw std::runtime_error("#### ERROR WRITING DATASET " + std::string(grpname) + "/" + std::string(name) +
                               " into " + std::string(filename) + ", error number " + std::to_string(status));
    }
  }
  H5Dclose(dataset);
  H5Sclose(dataspace);
...
void writeStringAttribute(hid_t handle, const char *buf, const char *attr_name)
{
  ...
  if (status < 0)
  {
    ...
    throw std::runtime_error("#### ERROR WRITING ATTRIBUTE " + std::string(grpname) + "/" + std::string(attr_name) +
                             " into " + std::string(filename) + ", error number " + std::to_string(status));
  }
}
