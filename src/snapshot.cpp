 ...
 */
{
  ...
  HBTReal RhoVirial = VirialFactor * Cosmology.Hz * Cosmology.Hz / 2.0 / PhysicalConst::G / ParticleMass *
                      Cosmology.ScaleFactor * Cosmology.ScaleFactor * Cosmology.ScaleFactor;
  for (i = np; i > 0; i--)
  {
    HBTReal r = RSorted[i - 1];
    if (i > RhoVirial * r * r * r)
      break;
  }
  Mvir = i * ParticleMass;
 ...
 */
{
  HBTReal RhoVirial = VirialFactor * Cosmology.Hz * Cosmology.Hz / 2.0 / PhysicalConst::G * Cosmology.ScaleFactor *
                      Cosmology.ScaleFactor * Cosmology.ScaleFactor;
 ...
 */
{
  ...
  ndiv = np; // guess mass
  rdiv = RSorted[ndiv - 1];
  rvir = pow(2.0 * PhysicalConst::G * ndiv * ParticleMass / VirialFactor / Cosmology.Hz / Cosmology.Hz, 1.0 / 3) /
         Cosmology.ScaleFactor; // guess radius
  while (true)
  {
    ...
    rdiv = rvir;
    rvir = pow(2.0 * PhysicalConst::G * ndiv * ParticleMass / VirialFactor / Cosmology.Hz / Cosmology.Hz, 1.0 / 3) /
           Cosmology.ScaleFactor; // recalibrate radius

    if (0 == ndiv || np == ndiv || fabs((rvir - rdiv) / rvir) < tol)
      break; // converged
  }
