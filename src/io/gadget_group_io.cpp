...
public:
  ...
  GroupFileReader_t(int SnapshotId)
    : SnapshotId(SnapshotId), iFile(0), NumberOfGroups(0), NumberOfParticles(0), Particles(), Len(), Offset()
  {
...
/*read_level: 0: meta data only ; 1: read group len and offset; 2: read particle list*/
{
  ...
  if (IsGroupV3)
    fseek(fd,
          sizeof(Ngroups) + sizeof(TotNgroups) + sizeof(Nids) + sizeof(TotNids) + sizeof(NFiles) + sizeof(fileoffset),
          SEEK_CUR);
  else
...
void Load(MpiWorker_t &world, int SnapshotId, vector<Halo_t> &Halos)
{
  ...
  if (world.rank() == 0)
  {
    for (int rank = 1; rank < world.size(); rank++)
      MPI_Send(HaloLenBuffer.data() + alltasks[rank].haloid_begin, alltasks[rank].nhalo, MPI_HBT_INT, rank, 0,
               world.Communicator);
  ...
  for (int i = 0, ireader = 0; i < world.size(); i++, ireader++)
  {
    ...
    if (i == world.rank()) // read
    {
      for (int iFile = thistask.ifile_begin; iFile < thistask.ifile_end; iFile++)
      {
        ...
        if (iFile == thistask.ifile_begin)
          start_particle = thistask.firstfile_begin_part;
        if (iFile == thistask.ifile_end - 1)
          end_particle = thistask.lastfile_end_part;
