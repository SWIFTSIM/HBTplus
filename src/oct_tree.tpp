    ...
    while (1)
    {
      ...
      else if (subid < NumberOfParticles) // a particle node, upgrade the node to internal
      {
        ...
        if (lenhalf < HBTConfig.TreeNodeResolutionHalf)
        {
          ...
          //~ lenhalf*2, Len, sub, i, Snapshot->GetComovingPosition(i][0], Snapshot->GetComovingPosition(i][1],
          //Snapshot->GetComovingPosition(i][2]);
        }
        else
        {
          sub = 0;
        ...
        nodeid = subid; // an internal node,take over it;
    }
  }
...
/* allocate tree memory to hold a maximum of max_num_part particles */
{
  ...
  if (MaxNumberOfCells < HBTConfig.TreeMinNumOfCells)
    MaxNumberOfCells = HBTConfig.TreeMinNumOfCells;
  Cells.clear();
