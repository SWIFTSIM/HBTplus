...
inline size_t fread_swap(void *buf, const size_t member_size, const size_t member_count, FILE *fp,
                         const bool FlagByteSwap)
{
  size_t Nread;
...
public:
   ...
   * T specify the input datatype. if T and U has the same size, read directly into outbuffer; otherwise the elements
   * are converted from type U to type T in a temporary buffer and then copied to outbuffer.
   */
  {
