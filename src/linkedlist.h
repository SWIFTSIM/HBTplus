...
public:
  ...
  virtual HBTInt restore_id(HBTInt i)
    const // has to make it virtual to be overriden by derived class. otherwise the derived class will use the wrong [].
  {
    return i;
  }
...
public:
  ...
  void SearchCylinderSerial(HBTReal radius_z, HBTReal radius_p, const HBTxyz &searchcenter,
                            ParticleCollector_t &collector);
};

class ReducibleCollector_t : public ParticleCollector_t
{
  virtual void Reduce(ParticleCollector_t &final_collector) = 0; // defines how to add the results from each thread
                                                                 // together into final_collector.
};
...
template <class REDUCIBLECOLLECTOR>
void LinkedlistPara_t::SearchShell(HBTReal rmin, HBTReal rmax, const HBTxyz &searchcenter,
                                   ParticleCollector_t &collector)
{ // parallel version. not suitable for use inside another parallel region. must specify a ReducibleCollector_t template
  // parameter to define Collect and Reduce method for each thread collector.
...
template <class REDUCIBLECOLLECTOR>
void LinkedlistPara_t::SearchCylinder(HBTReal radius_z, HBTReal radius_p, const HBTxyz &searchcenter,
                                      ParticleCollector_t &collector)
{ // parallel version. not suitable for use inside another parallel region. must specify a ReducibleCollector_t template
  // parameter to define Collect and Reduce method for each thread collector.
#pragma omp parallel for
...
/* a simple collector to be used for parallel search; it keeps a local vector to store located particles in each thread,
 * and then dump them to the final output collector*/
{
public:
