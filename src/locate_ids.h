...
#pragma omp atomic
    ...
    target_ids.resize(total_nr_recv);
    {
      ...
      // Exchange IDs
      Pairwise_Alltoallv(ids_sendbuf, sendcounts, senddispls, MPI_HBT_INT, target_ids, recvcounts, recvdispls,
                         MPI_HBT_INT, comm);
    }
    ...
    target_values.resize(total_nr_recv);
    {
      ...
      // Exchange values
      Pairwise_Alltoallv(values_sendbuf, sendcounts, senddispls, mpi_value_type, target_values, recvcounts, recvdispls,
                         mpi_value_type, comm);
    }
  }
...
#pragma omp atomic
    ...
    imported_ids_to_find.resize(total_nr_recv);
    {
      ...
      Pairwise_Alltoallv(ids_sendbuf, imported_sendcounts, imported_senddispls, MPI_HBT_INT, imported_ids_to_find,
                         imported_recvcounts, imported_recvdispls, MPI_HBT_INT, comm);
    }
  }
...
#pragma omp parallel for schedule(static, 10 * 1024)
  ...
  Pairwise_Alltoallv(match_count, imported_recvcounts, imported_recvdispls, MPI_HBT_INT, count_found,
                     imported_sendcounts, imported_senddispls, MPI_HBT_INT, comm);
  //
...
#pragma omp parallel for
  for (int rank_nr = 0; rank_nr < comm_size; rank_nr += 1)
  {
    ...
    for (HBTInt i = imported_recvdispls[rank_nr]; i < imported_recvdispls[rank_nr] + imported_recvcounts[rank_nr];
         i += 1)
    {
  ...
  // Exchange result values
  {
    ...
    // Exchange data
    Pairwise_Alltoallv(result_sendbuf, result_sendcounts, result_senddispls, mpi_value_type, values_found,
                       result_recvcounts, result_recvdispls, mpi_value_type, comm);
  }
