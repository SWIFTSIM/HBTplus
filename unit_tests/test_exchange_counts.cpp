...
int main(int argc, char *argv[]) {
  ...
  MPI_Allgather(sendcounts.data(), comm_size, MPI_INT,
                all_sendcounts.data(), comm_size, MPI_INT,
                MPI_COMM_WORLD);
  std::vector<int> all_recvcounts(comm_size*comm_size);
  MPI_Allgather(recvcounts.data(), comm_size, MPI_INT,
                all_recvcounts.data(), comm_size, MPI_INT,
                MPI_COMM_WORLD);
